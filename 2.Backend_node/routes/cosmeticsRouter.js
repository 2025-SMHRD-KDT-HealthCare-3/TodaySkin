...
router.get('/search', requireLogin, async (req, res, next) => {
    try {
        ...
        if (results.length === 0) {
            throw new ValidationError('해당 화장품을 찾을 수 없습니다.', 404);
        }
...
router.get('/recommend', requireLogin, async (req, res, next) => {
    try {
        ...
        const [results] = await conn.query(recommendedSql, [user_no, user_no]);

        res.json({
            status: "success",
            data: {
                recommendation: results,
                message: results.length === 0 ? "현재 루틴에 필요한 모든 화장품을 보유하고 계시네요!" : null
            }
        });
    } catch (error) {
        next(error);
...
router.post('/user-cosmetics', requireLogin, async (req, res, next) => {
    try {
        ...
        const sql = `
            ...
            VALUES (?, ?, ?, NOW())
        `;
        await conn.query(sql, [user_no, cos_no, expired_at || null]);
...
router.delete('/user-cosmetics/:ucos_no', requireLogin, async (req, res, next) => {
    try {
        ...
        if (!ucos_no || isNaN(ucos_no)) throw new ValidationError("올바른 화장품 번호를 입력해주세요.");
