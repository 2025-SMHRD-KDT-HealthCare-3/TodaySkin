...
router.post('/user-cosmetics', requireLogin, async (req, res, next) => {
    try {
        ...
        // 1. 이미 등록된 내역(보유 또는 추천)이 있는지 먼저 확인합니다.
        const [existing] = await conn.query(
            "SELECT ucos_no, source FROM user_cosmetics WHERE user_no = ? AND cos_no = ?",
            [user_no, cos_no]
        );
        ...
        if (existing.length > 0) {
            ...
            if (existing[0].source === '추천') {
                ...
                return res.json({
                    status: "success",
                    data: { message: "추천받은 화장품을 보유 목록으로 전환했습니다!" }
                });
            } 
