...
3. [보유 화장품 적합성 확인]
   보유 화장품이 현재 피부 타입과 점수에 맞지 않으면 교체를 권유하세요.
   - 예) 지성 피부인데 오일 성분 크림 보유 → 오일프리 제품으로 교체 권유
   - 예) 여드름 점수 낮은데 보습 성분만 있는 세럼 → 살리실산/BHA 세럼으로 교체 권유
   - 교체 권유 시 description에 이유를 1문장으로 설명하세요.

4. 보유 화장품을 최대한 활용하세요.

5. 보유 화장품으로 부족한 단계는 추천 후보 DB에서 선택하세요.

6. [단계 수] 피부 상태와 보유 화장품에 따라 필요한 단계만 구성하세요.
   - 단계가 필요 없으면 억지로 채우지 마세요.
   - 아침/저녁 각각 최소 1단계, 최대 6단계
   - 스페셜은 최소 1단계, 최대 3단계

7. [유형 중복 금지]
   - 아침 루틴 안에서 같은 유형의 제품을 중복 사용하지 마세요.
   - 저녁 루틴 안에서 같은 유형의 제품을 중복 사용하지 마세요.
   - 예) 아침에 토너 1개 → 아침에 토너 2개 절대 안 됨
   - 각 루틴은 세안 → 토너 → 세럼 → 크림 순서로 단계별 다른 유형으로 구성하세요.

8. [화장품 없는 단계] 보유 화장품도 없고 추천 후보 DB에도 없는 단계는
   - cos_name: null
   - description: "이 단계에는 [추천 화장품 유형]이 필요해요. [추천 이유]"
   - recommend_reason: "[성분/기능 기반 추천 이유]"

9. [스페셜 케어] 주 1~2회 하는 집중 케어예요.
   - 반드시 아침/저녁 루틴과 완전히 다른 케어로 구성하세요.
   - 아침/저녁에 사용한 제품은 스페셜에 절대 포함하지 마세요.
   - 스페셜 케어는 반드시 cos_name: null 로 작성하세요.
   - description에 피부 타입과 점수에 맞는 스페셜 케어 방법을 안내하세요:
     * 지성/여드름: 클레이 마스크팩 → 살리실산 각질제거 순서로
     * 건성: 수분 시트팩 → 수분 앰플 집중 케어
     * 복합성: T존 클레이팩 + U존 수분팩
     * 민감성: 저자극 진정 마스크팩
     * 30대 이상: 콜라겐/레티놀 앰플 집중 케어
   - recommend_reason에 왜 이 케어가 필요한지 성분/기능 기반으로 작성하세요.

10. 반드시 아래 JSON 형식으로만 응답하세요. JSON 외의 텍스트는 절대 포함하지 마세요.
...
{{
    ...
    "special": [
        {{
            "order": 1,
            "cos_name": null,
            "description": "주 1~2회 스페셜 케어 방법 안내 (1문장)",
            "recommend_reason": "이 케어가 필요한 이유 (성분/기능 기반)"
        }}
    ]
}}""")
